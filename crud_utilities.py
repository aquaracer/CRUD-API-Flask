...
def get_city_json(track_id, direction):
    ...
    сity_of_flight = ans[0][0] # получили город отправления(или прибытия в зависимости от переменной direction
...
def get_weather_from_yandex(date_time, city_name):
    """Входные данные в функцию: дата_время и название города.
       ...
       Также производит кеширование(путем вызова функции add_data_to_cache) JSON с погодой на 7 дней для заданного города"""
