...
if len(ans) < 1:# если в массиве что-то есть значит заданный город есть в базе
    flag = False
    return flag
else:
    ...
    if actual_year - cache_year > 0:
        ...
        actual_month = int(actual_time_date[5:7])
        cache_month = int(cache_time_date[5:7])
        actual_day = int(actual_time_date[8:10])
        cache_day = int(cache_time_date[8:10])
        actual_hour = int(actual_time_date[11:13])
        cache_hour = int(cache_time_date[11:13])
        aim_track_id = city + cache_time_date  # получаем целевой Track_id для получения нужного Json bp КЭША
        json = 
        if actual_month - cache_month >= 2:
            flag = False
            return flag
        elif actual_month - cache_month == 1:
            months_1 = [4,6,9,11]
